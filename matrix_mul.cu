...
__global__ void matrix_vector product(float *A, float *v1, float *v2, int matrix_size) {
    int row = bloackIdx.x * blockDim.x +threadIdx.x;
    int col = bloackIdx.y * blockDim.y +threadIdx.y;
...
int main(int argc, char* argv[]) {
    ...
    dim3 grid_shape = dim3(max(1.0, ceil((float)matrix_size/(float)block_shape.x)),max(1.0, ceil((float)matrix_size/(float)block_shape.y)));
